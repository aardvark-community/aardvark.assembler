...
type ExecutableMemory private() =
    ...
    static let mutable pageSize = 0un

    static member PageSize = 
        if pageSize = 0un then
    ...
    static member Free(ptr : nativeint, size : unativeint) =
        efree(ptr, size)

    static member Copy(src : nativeint, dst : nativeint, size : unativeint) =
        ecpy(dst, src, size)
...
type Arm64Stream() =
    ...
    let jumpIndices = Dict<AssemblerLabel, Dict<int, voption<JumpCondition>>>()
    ...
    member x.Finish() =
        let res = System.Memory<uint32>(data, 0, count)
        data <- null
        count <- 0
        res
    ...
    interface IAssemblerStream with
        ...
        member x.Mark(label : AssemblerLabel) =
            ...
            l.Position <- int64 count * 4L

            match jumpIndices.TryRemove label with
            | (true, indices) ->    
                let o = count
                for KeyValue(i, cond) in indices do
                    count <- i
                    let off = int label.Position - count * 4
                    match cond with
                    | ValueNone -> x.jmp(off)
...
let main _ =
    ...
    let ptr = ExecutableMemory.Alloc 32768un

    let code = new Arm64Stream()
    let s = code  :> IAssemblerStream
    ...
    let h = code.Pin()
    ExecutableMemory.Copy(NativePtr.toNativeInt (NativePtr.ofVoidPtr<byte> h.Pointer), ptr, unativeint (sizeof<uint32> * code.Length))
    h.Dispose()

    let a = Marshal.GetDelegateForFunctionPointer<IntDelegate>(ptr)
    a.Invoke() |> printfn "result: %d"


    NativePtr.ofNativeInt<uint32> store |> NativePtr.read |> printfn "store[0]: %d"
    ExecutableMemory.Free(ptr, 32768un)
