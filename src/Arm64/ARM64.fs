...
type ExecutableMemory private(ptr : nativeint, length : unativeint, capacity : unativeint) =
    [<DllImport("jitmem")>]
    ...
    static member Alloc(size : unativeint) =
        let ps = ExecutableMemory.PageSize
        let effectiveSize =
            if size % ps = 0un then size
            else (1un + size / ps) * ps
        let ptr = ealloc effectiveSize
        new ExecutableMemory(ptr, size, effectiveSize)

    member x.Realloc(newSize : unativeint) =
        let ps = ExecutableMemory.PageSize
        let newEffectiveSize =
            if newSize % ps = 0un then newSize
            else (1un + newSize / ps) * ps

        if newEffectiveSize <> capacity then
            if newEffectiveSize = 0un then
                efree(ptr, capacity)
                ptr <- 0n
                length <- 0un
                capacity <- 0un
            else
                let newPtr = ealloc newEffectiveSize
                if ptr <> 0n then
                    ecpy(newPtr, ptr, min newSize length)
                    efree(ptr, capacity)
                ptr <- newPtr
                capacity <- newEffectiveSize
                length <- newSize

    member x.Pointer = ptr

    member x.Write(data : Memory<'a>, offset : nativeint) =
        let size = unativeint sizeof<'a> * unativeint data.Length
        if offset < 0n then raise <| new ArgumentOutOfRangeException("offset")
        if unativeint offset + size > length then raise <| new ArgumentOutOfRangeException("size")

        use src = data.Pin()
        let pSrc = src.Pointer |> NativePtr.ofVoidPtr<byte> |> NativePtr.toNativeInt
        ecpy(ptr + offset, pSrc, size)
